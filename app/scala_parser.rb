...
class ScalaParser < Parslet::Parser
  ...
  rule(:trait_expr) { word_trait >> space >> class_name >> space? }
  rule(:class_expr) { space? >> (word_case >> space).maybe >> word_class >>
    space >> class_name.as(:current_class) >> space? >> (lparen >> var_expressions.maybe >> rparen).maybe >> parent_classes.maybe}
  rule(:some_expr) { package_expr.maybe | (import_expr | trait_expr | class_expr).repeat }
  root(:some_expr)
end
